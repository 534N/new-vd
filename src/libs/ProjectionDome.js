...
function ProjectionDome( parentEl, w, h, videoEl, options ) {
    ...
    if ( !isNaN(this.options.paddingX) || !isNaN(this.options.paddingY) ) {
        this.setPadding({
            x:  this.options.paddingX,
            y:  this.options.paddingY
        });
    }
...
ProjectionDome.prototype = {
        ...
        var createAnimation = function () {
            ...
            self.onloadeddata = function (d) {
                ...
                self._uniforms = {
                    ...
                    paddingX:     { type:  'f', value:  self._paddingX },
                    paddingY:     { type:  'f', value:  self._paddingY }
                };
    ...
    onMouseWheel: function(event) {
        ...
        var zoomRatio = (this.zoom - this._cam_z.min) / (this._cam_z.max - this._cam_z.min);
        
        this._mouseSensitivityX = this.options.mouseSensitivityX - (1 - zoomRatio) * 0.15
        this._mouseSensitivityY = this.options.mouseSensitivityX - (1 - zoomRatio) * 0.15
    ...
    render: function() {
        ...
        this._uniforms.paddingX.value = this._paddingX;
        this._uniforms.paddingY.value = this._paddingY;
    ...
    setPadding: function( padding ) {
        if ( typeof padding != typeof {} ) {
            console.error('[ProjectionDome.setPadding]  invalid params');
            return;
        }

        if ( !isNaN(padding.x) ) {
            this._paddingX = padding.x;
        }

        if ( !isNaN(padding.y) ) {
            this._paddingY = padding.y;
        }
    },
...
ProjectionDome.fragmentShader2 = `
    ...
    void main() {
        ...
        float _ratio = (paddingY != 1.0 || paddingX != 1.0) ? 1.0 : ratio;

        if (paddingX > 0.0 && paddingY > 0.0) {
            d_uv.x = (uv.x - 0.5) * _ratio * paddingX + 0.5;
            d_uv.y = (uv.y - 0.5) * paddingY + 0.5;
        }
